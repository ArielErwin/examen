...
import utils.ConfigGrades;
	...
	public class Management {
	      ...
	      public ArrayList<Student> ShowOrderZA(){
	    	  mainlist.sort((Student a , Student b)->{
	      ...
	      public ArrayList<Student> ShowMejoresParciales(){
	    	  ArrayList<Student> mejoresparciales = new ArrayList<Student>();
	    	  for(int i=0 ; i< mainlist.size(); i++) {
	    		  if (mainlist.get(i).getGrade().getTOTALGRADE()>= ConfigGrades.MAX_GRADES) {
	    			  mejoresparciales.add(mainlist.get(i));
	    		  }		  
	    	  }
	    	 return mejoresparciales;  
	      }
